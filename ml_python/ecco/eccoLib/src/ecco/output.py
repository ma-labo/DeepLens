...
class NMF:
    ...
    def explore(self, input_sequence: int = 0, filter_token: bool = False, top_k: int = -1, **kwargs):
        ...
        if filter_token:
            ...
            for f in factors:
                idx = np.argmax(f)
                if self.tokens[input_sequence][idx] in string.punctuation or self.tokens[input_sequence][idx] == '[SEP]':
                    pass
                elif self.tokens[input_sequence][idx] in sw and idx < len(self.tokens[input_sequence]) - 1 and self.tokens[input_sequence][idx+1][:2] != '##':
                    pass
                else:
                    ...
                    if top_k == -1:
                        ...
                        for ind in np.argsort(f)[::-1][:top_k]:
                            tmp_f[ind] = f[ind]
                        tmp.append(tmp_f)
            factors = tmp if tmp != [] else factors
            # post process
            new_tokens = []
            new_factors = []
            new_idx = 0
            for (idx, token) in enumerate(tokens):
                type = "input" if idx < self.n_input_tokens else 'output'
                if token['token'] == '[CLS]' or token['token'] == '[SEP]':
                    continue
                else:
                    if token['token'][:2] != '##':
                        new_tokens.append({'token': token['token'],
                               'token_id': int(self.token_ids[input_sequence][idx]),
                               # 'token_id': int(self.token_ids[idx]),
                               'type': type,
                               # 'value': str(components[0][comp_num][idx]),  # because json complains of floats
                        ...
                        new_factors.append(np.array([f[idx] for f in factors]))
                        new_idx += 1
                    else:
                        new_tokens[-1]['token'] = new_tokens[-1]['token'] + token['token'][2:]
                        new_factors[-1] += np.array([f[idx] for f in factors])

            new_factors = np.array(new_factors).T.tolist()
            tokens = new_tokens
            factors = new_factors
